package com.geeks.mylocker.data;
...
public class LogHistory implements Parcelable{
    ...
    private final List<Pair<String, Long>> mData = new ArrayList<Pair<String, Long>>();
    ...
    public List<Pair<String, Long>> getData() {
        return new ArrayList<Pair<String, Long>>(mData);
    }
    ...
    private LogHistory(Parcel in) {
        ...
        for (int i = 0; i < texts.length; i++) {
            Pair<String, Long> pair = new Pair<String, Long>(texts[i], timestamps[i]);
            mData.add(pair);
        }
    }
}
