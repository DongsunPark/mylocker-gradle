...
public class EncryptionActivity extends Activity implements OnClickListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pbe);
        setProgressBarIndeterminateVisibility(false);

        derivationMethodSpinner = findById(R.id.pbe_derivation_method_spinner);
        derivationMethodSpinner.setOnItemSelectedListener(this);
        encryptor = PADDING_ENCRYPTOR;
        derivationMethodSpinner.setSelection(0);

        passwordText = findById(R.id.pbe_password_text);

        encryptedText = findById(R.id.pbe_encrypted_text);
        decryptedText = findById(R.id.pbe_decrypted_text);
        rawKeyText = findById(R.id.pbe_raw_key_text);
